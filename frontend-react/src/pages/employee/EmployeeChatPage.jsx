...
import { Send, Bot, User, Trash2, LayoutDashboard } from 'lucide-react';
...
function inlineMd(text) {
  return text
    ...
    .replace(
      /`([^`]+)`/g,
      '<code style="background:rgba(255,255,255,0.1);padding:0.1em 0.35em;border-radius:4px;font-size:0.85em;font-family:monospace">$1</code>'
    );
}
...
function buildResponseHTML(r) {
  ...
  if (r.table) {
    html += `<div style="margin-top:8px;border-radius:8px;overflow:hidden;border:1px solid rgba(48,54,61,0.8)"><table style="width:100%;border-collapse:collapse;font-family:monospace;font-size:11px"><thead><tr>${r.table.cols
      .map(
        (c) =>
          `<th style="background:rgba(13,17,23,0.95);padding:7px 10px;text-align:left;color:#8b949e;font-size:9px;letter-spacing:1px;text-transform:uppercase;border-bottom:1px solid rgba(48,54,61,0.8)">${c}</th>`
      )
      .join('')}</tr></thead><tbody>${r.table.rows
        .map(
          (row) =>
            `<tr>${row
              .map(
                (c) =>
                  `<td style="padding:7px 10px;border-bottom:1px solid rgba(48,54,61,0.4);color:#c9d1d9">${c}</td>`
              )
              .join('')}</tr>`
        )
  ...
  if (r.code) {
    html += `<div style="background:rgba(13,17,23,0.95);border:1px solid rgba(48,54,61,0.8);border-radius:8px;padding:10px 12px;margin-top:8px;font-family:monospace;font-size:10px;color:#a5b4fc;overflow-x:auto"># Generated query\n${r.code}</div>`;
  }
  if (r.insight) {
    html += `<div style="margin-top:10px;padding:10px 12px;background:rgba(88,166,255,0.06);border-left:3px solid var(--primary);border-radius:0 8px 8px 0;font-size:12px;color:var(--text-main);line-height:1.6"><strong style="color:var(--primary)">✦ Insight: </strong>${r.insight}</div>`;
  }
  return html;
}
...
const EmployeeChatPage = () => {
  ...
  const [messages, setMessages] = useState([
    {
      ...
      role: 'ai',
      content:
        "👋 Hello! I'm your **AI Data Assistant**. Ask me about **totals**, **trends**, **top performers**, **anomalies**, **forecasts**, or anything about your data!\n\nSelect a dataset from the dropdown above and try one of the suggested questions on the left →",
    ...
    const loadDatasets = async () => {
      try {
        ...
        if (res.success && res.data) {
          const readyDatasets = res.data.filter(
            (d) => d.status === 'completed' || d.status === 'ready' || d.status === 'cleaned'
          );
          setAvailableDatasets(readyDatasets);

          if (!datasetId && readyDatasets.length > 0) {
            const first = readyDatasets[0];
            setSelectedDataset(first);
            fetchDatasetInfo(first.dataset_id || first.id);
          } else if (datasetId) {
            const selected = readyDatasets.find((d) => String(d.dataset_id || d.id) === String(datasetId));
            if (selected) {
              setSelectedDataset(selected);
              fetchDatasetInfo(selected.dataset_id || selected.id);
            }
          }
        }
      } catch (err) {
  ...
  const handleSend = useCallback(
    async (text) => {
      ...
      setMessages((prev) => [
        ...prev,
        { id, role: 'user', content: msg, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
      ]);
      setIsLoading(true);
        ...
        if (dsId) {
          ...
          setMessages((prev) => [
            ...prev,
            {
              id: bid,
              role: 'ai',
              content: buildResponseHTML({ text: `${answer}${suggested}`, require_permission: response?.require_permission }),
              time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
          ]);
        } else {
          ...
          setMessages((prev) => [
            ...prev,
            { id: bid, role: 'ai', content: buildResponseHTML(r), time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
          ]);
        }
      } catch (err) {
    ...
    setMessages([
      {
        id: 0,
        role: 'ai',
        content: 'Chat cleared. Ask a new question about your dataset.',
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }
    ]);
    msgId.current = 1;
  ...
  const colTypeColors = {
    numeric: { border: 'rgba(63,185,80,0.2)', color: 'var(--success)', label: 'NUM' },
    categorical: { border: 'rgba(188,140,255,0.2)', color: 'var(--accent)', label: 'CAT' },
    date: { border: 'rgba(210,153,34,0.2)', color: 'var(--warning)', label: 'DATE' }
  };

  const liveColumns =
    datasetInfo?.headers?.map((h) => ({
      name: h,
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 14 }}>
              ...
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: 'var(--text-muted)', lineHeight: 1.8 }}>
                {datasetInfo ? (
                  <>
                    ...
                    <span style={{ color: 'var(--primary)' }}>{datasetInfo.headers.length}</span> cols
                    <br />
            ...
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, color: 'var(--text-muted)', marginBottom: 6 }}>
              Click to reference in query
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {smartSuggestions.map((s, i) => (
                ...
                >
                  <span style={{ fontSize: 11, marginRight: 5 }}>{s.icon}</span>
                  {s.q}
                </div>
              ))}
            </div>
            ...
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: 'var(--text-muted)', lineHeight: 2 }}>
              {queryHistory.map((q, i) => (
                <div key={i} style={{ cursor: 'pointer', color: 'var(--text-main)' }} onClick={() => handleSend(q)}>
                  → {q}
                </div>
              ))}
            </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0, overflow: 'hidden' }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              {availableDatasets.length > 1 ? (
                <select
                  ...
                  onChange={(e) => {
                    const ds = availableDatasets.find((d) => String(d.dataset_id || d.id) === e.target.value);
                    if (ds) {
                      ...
                      setMessages([
                        {
                          id: 0,
                          role: 'ai',
                          content: `👋 Switched to **${ds.name}**. Ask me about **totals**, **trends**, **top performers**, or anything about your data!`,
                          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                        }
                      ]);
                      msgId.current = 1;
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button className="emp-btn emp-btn-ghost emp-btn-sm" onClick={() => navigate('/employee/dashboard')}>
          ...
          >
            {messages.map((msg) => {
              ...
              const isImageError =
                !isUser &&
                msg.content &&
                (msg.content.toLowerCase().includes('does not support image') ||
                  msg.content.toLowerCase().includes('cannot read image') ||
                  msg.content.toLowerCase().includes('image input not supported'));
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    justifyContent: isImageError ? 'center' : 'flex-start'
                  }}
                >
                  {!isImageError && (
                    <div
                  ...
                  <div style={{ maxWidth: isImageError ? '70%' : '75%', minWidth: 0 }}>
                    {isImageError ? (
                      <div
                        style={{
                          padding: '20px 24px',
                      ...
                      <div
                        style={{
                          ...
                          background: isUser
                            ? 'linear-gradient(135deg, rgba(31,111,235,0.25), rgba(88,166,255,0.15))'
                            : 'rgba(22,27,34,0.85)',
